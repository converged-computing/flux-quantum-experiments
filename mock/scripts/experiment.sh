...
#
# Arms
#   inline       one ordinary job. It takes its nodes, then opens the session
...
#   sessionfirst acquire priority, then submit the classical, which queues
...
have_row() {
    ...
    awk -F, -v e="$1" -v a="$2" -v d="$3" -v l="$4" -v s="$5" -v t="$6" '
        $1 == e && $2 == a && $3 == d && $4 == l && $5 == s && $6 == t {
            found = 1; exit
        }
        END { exit !found }' "$OUT"
}
...
wait_clean() {
    ...
    say "TIMEOUT on $1"; flux jobs -a >&2; return 1
}
drain() {
...
start_load() {
    ...
    flux submit --quiet -n"$n" sleep "$secs" >/dev/null 2>&1
    sleep 1
}
...
HDR="exp,arm,depth,load_pct,size,trial,t0,submit,priority,alloc,start,finish,scout_start,scout_finish,bg_total,bg_done,work_s,load_cores"
...
trial() {
    ...
    # e4 places the load by core count so its spare figure is exact
    [ -n "${E4_LOAD_CORES:-}" ] && [ "$exp" = e4 ] && load_cores="$E4_LOAD_CORES"

    if have_row "$exp" "$arm" "$depth" "$load" "$size" "$t"; then
        say "skip $exp $arm depth=$depth load=$load size=$size trial=$t, already measured"
        return 0
    fi
    local tag="bg-$$-$RANDOM"
    ...
    start_load "$load_cores" $(( OVERHEAD + depth * 1 + WORK + 10 ))
    start_stream "$tag"
    ...
    case "$arm" in
    inline)
        # holds its nodes for the whole vendor wait
        local main
...
print('done', s)")
    ...
    coscheduled|nowarmup)
        ...
        [ -z "$main" ] && { say "no classical job"; return 1; }
        wait_clean "$main" || return 1
        wait_clean "$scout" || return 1
    ...
    sessionfirst)
        # acquire priority first, then submit and let it queue
        local session main
        session=$(timeout "$TIMEOUT" flux python -c "
import json
from flux_quantum.backends import get_backend
b = get_backend('mock')
o = json.loads('$(qopts "$depth")')
s = b.open_session(o)
b.wait_for_priority(o)
print(s)" 2>/dev/null | tail -1)
        priority=$(date +%s.%N)
        [ -z "$session" ] && { say "no session"; return 1; }
        main=$(QUANTUM_SESSION_ID="$session" flux submit -n"$size" \
                 -- sh -c "sleep $WORK; echo got \$QUANTUM_SESSION_ID")
        say "$exp $arm depth=$depth load=$load size=$size trial=$t main=$main"
        wait_clean "$main" || return 1
        submit=$(ev "$main" submit); alloc=$(ev "$main" alloc)
        start=$(ev "$main" start); finish=$(ev "$main" finish)
        s_start="$priority"; s_finish=$(date +%s.%N)
        ;;
    esac

    bg_done=$(stream_drained "$tag")
    row "$exp,$arm,$depth,$load,$size,$t,$t0,$submit,$priority,$alloc,$start,$finish,$s_start,$s_finish,$bg_total,$bg_done,$WORK,$load_cores"
    drain
}
...
run_e1() {   # what the allocation is held for, against vendor queue depth
    for depth in ${DEPTHS:-0 7 14 100 491}; do
        for t in $(seq 1 "$TRIALS"); do
            for arm in $(shuffled inline coscheduled sessionfirst); do
...
run_e3() {   # node seconds wasted against allocation size
    for size in ${SIZES:-1 8 32 64 96}; do
        for t in $(seq 1 "$TRIALS"); do
            for arm in $(shuffled inline coscheduled); do
...
case "$WHAT" in
    ...
    *) echo "usage: $0 {e1|e2|e3|e4|all}" >&2; exit 2 ;;
esac
